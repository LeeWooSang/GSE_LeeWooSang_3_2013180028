...
SceneManager::SceneManager(int width, int height)
{
	...
	for (int i = 0; i < MAX_BUILDINGS_COUNT; ++i)
		m_pBuildings[i] = NULL;
...
void SceneManager::Init_Building(void)
{
	...
	// 블루팀 빌딩 인덱스(0~2), 레드팀 빌딩 인덱스 (3~5)
	for (int i = 0; i < MAX_BUILDINGS_COUNT; ++i)
	{
		if (i < 3)
		{
			if (m_pBuildings[i] == NULL)
			{
				m_pBuildings[i] = new Object(0, 0);

				m_pBuildings[i]->Get_X(-150.0 + (150 * index));
				m_pBuildings[i]->Get_Y(-300.0);
				m_pBuildings[i]->Get_Z(0.0);
				m_pBuildings[i]->Get_Size(100.0);
				m_pBuildings[i]->Get_Color_R(1.0);
				m_pBuildings[i]->Get_Color_G(0.0);
				m_pBuildings[i]->Get_Color_B(0.0);
				m_pBuildings[i]->Get_Color_A(1.0);
				m_pBuildings[i]->Get_TeamType(BLUE_TEAM);
				m_pBuildings[i]->Get_TextureID(m_pRenderer->CreatePngTexture("./Textures/PNGs/castle6.png"));
			}
			++index;
		}
		else
		{
			if (m_pBuildings[i] == NULL)
			{
				...
				m_pBuildings[i]->Get_Z(0.0);
				m_pBuildings[i]->Get_Size(100.0);
				m_pBuildings[i]->Get_Color_R(0.0);
				m_pBuildings[i]->Get_Color_G(0.0);
				m_pBuildings[i]->Get_Color_B(1.0);
				m_pBuildings[i]->Get_Color_A(1.0);
				m_pBuildings[i]->Get_TeamType(RED_TEAM);
				m_pBuildings[i]->Get_TextureID(m_pRenderer->CreatePngTexture("./Textures/PNGs/castle3.png"));
			}
			++index;
		}
...
void SceneManager::Draw_Building(void)
{
	...
	for (int i = 0; i < MAX_BUILDINGS_COUNT; ++i)
	{
		...
		{
			m_pRenderer->DrawTexturedRect(
				m_pBuildings[i]->Set_X(),
				m_pBuildings[i]->Set_Y(),
				m_pBuildings[i]->Set_Z(),
				m_pBuildings[i]->Set_Size(),
				m_pBuildings[i]->Set_Color_R(),
				m_pBuildings[i]->Set_Color_G(),
				m_pBuildings[i]->Set_Color_B(),
				m_pBuildings[i]->Set_Color_A(),
				m_pBuildings[i]->Set_TextureID());
		}
	}
}
...
void SceneManager::Delete_Building(int index)
{
	delete m_pBuildings[index];
	m_pBuildings[index] = NULL;
}
